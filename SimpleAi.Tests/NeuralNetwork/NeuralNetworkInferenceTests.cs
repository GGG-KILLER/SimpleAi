...
public class NeuralNetworkInferenceTests
{
    ...
    public void NeuralNetworkX2ERunInference_Throws_exception_when_input_size_is_wrong()
    {
        ...
        Assert.Throws<ArgumentException>(() =>
            network.RunInference(session, [], stackalloc float[2]));
        Assert.Throws<ArgumentException>(() =>
            network.RunInference(session, [1], stackalloc float[2]));
        Assert.Throws<ArgumentException>(() =>
            network.RunInference(session, [1, 1, 1], stackalloc float[2]));
    }
    ...
    public void NeuralNetworkX2ERunInference_Throws_exception_when_output_size_is_wrong()
    {
        ...
        Assert.Throws<ArgumentException>(() =>
            network.RunInference(session, [1, 1], []));
        Assert.Throws<ArgumentException>(() =>
            network.RunInference(session, [1, 1], [1]));
        Assert.Throws<ArgumentException>(() =>
            network.RunInference(session, [1, 1], [1, 1, 1]));
    }
