...
/// <summary>The base interface for a training data storage.</summary>
...
{
    /// <summary>The total number of items contained within this provider.</summary>
    [PublicAPI]
    int Length { get; }

    /// <summary>Obtains a single training data point.</summary>
    /// <param name="index">The index to obtain data at.</param>
    [PublicAPI]
    TrainingDataPoint<T> this[Index index] { get; }

    /// <summary>Obtains a slice of the training data.</summary>
    /// <param name="range">The range of values to obtain data from.</param>
    [PublicAPI]
    ReadOnlyMemory<TrainingDataPoint<T>> this[Range range] { get; }

    /// <summary>Shuffles the training data randomly.</summary>
    [PublicAPI]
    void Shuffle();
}

/// <summary>A training data storage stored entirely in memory without any special processing.</summary>
/// <remarks>The constructor creates a copy of the data to prevent mutations.</remarks>
...
public sealed class InMemoryTrainingData<T>(IEnumerable<TrainingDataPoint<T>> trainingData) : ITrainingData<T>
{
    ...
    /// <summary>Returns an enumerator that enumerates through the contents of the training data.</summary>
